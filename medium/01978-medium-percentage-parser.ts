...
/**
 * A extends ${infer Sign extends '+' | '-'}${infer Number}% ? [Sign, Number, '%']：检查 A 是否以 + 或 - 开始，后跟数字和 %。如果是，提取符号、数字和百分号。
 *
 * A extends ${infer Sign extends '+' | '-'}${infer Number} ? [Sign, Number, '']：检查 A 是否以 + 或 - 开始，后跟数字。如果是，提取符号和数字。
 *
 * 其他情况类似，但针对不带符号或不带百分号的情况。
 */
type PercentageParser<A extends string> =
  A extends `${infer Sign extends '+' | '-'}${infer Number}%` ? [Sign, Number, '%']
    : A extends `${infer Sign extends '+' | '-'}${infer Number}` ? [Sign, Number, '']
      : A extends `${infer Number}%` ? ['', Number, '%']
        : A extends `${infer Number}` ? ['', Number, '']
          : ['', '', ''];
